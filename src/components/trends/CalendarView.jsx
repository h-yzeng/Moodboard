export function CalendarView({ calendarDays, getMoodEmoji, viewMode }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-7 gap-3 mb-2 text-center text-xs font-semibold text-gray-600" aria-hidden="true">
      ...
      <div className="grid grid-cols-7 gap-3" role="grid" aria-label="Mood calendar">
        {calendarDays.map((day, idx) => {
          ...
          return (
            <div
              ...
              role="gridcell"
              aria-label={label}
              className={`rounded-2xl p-3 border shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-lg ${
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                {day.isToday && <span className="text-[10px] font-bold text-blue-600">Today</span>}
              </div>
              <div className="min-h-9 flex flex-col gap-2">
                {day.mood ? (
                  <div className="inline-flex items-center gap-2 px-2 py-1 rounded-full bg-white border border-gray-100 text-sm font-semibold shadow-sm">
                    <span aria-hidden="true">{getMoodEmoji(day.mood.mood)}</span>
                    <span className="capitalize">{day.mood.mood}</span>
                  </div>
                ) : (
                  <span className="text-xs text-gray-400">No entry</span>
                )}
                {day.mood?.note && <p className="text-[11px] text-gray-600 line-clamp-2">“{day.mood.note}”</p>}
              </div>
            </div>
          );
        })}
